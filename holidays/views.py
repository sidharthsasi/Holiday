...
def search_holidays(request):
    ...
    try:
        ...
        year_to_fetch = selected_year or datetime.now().year
        ...
        if search_query:
            holidays = [h for h in holidays if search_query in h.get('name', '').lower()]

        if selected_month:
            holidays = [h for h in holidays if h.get('date', {}).get('datetime', {}).get('month') == int(selected_month)]

        unique_holidays = {}
        for holiday in holidays:
            holiday_name = holiday.get('name', '').lower()
            if holiday_name not in unique_holidays:
                unique_holidays[holiday_name] = holiday
        
        holidays = list(unique_holidays.values())

    except requests.exceptions.RequestException:
        error_message = "Could not fetch holidays"
    except (ValueError, KeyError, TypeError):
        error_message = "There is an issue processing the data."
